...
class TermuxPlugin {
  ...
  static const String description = 'Execute commands via Termux SSH (local/network)';
  ...
  static List<Tool> get tools => [
    Tool(
      ...
      parameters: {
        'command': ToolParameter(name: 'command', description: 'Command to execute', type: ToolParameterType.string, required: true),
        'timeout': ToolParameter(name: 'timeout', description: 'Timeout in seconds', type: ToolParameterType.int),
      },
      requiresAuth: false,
    ),
    Tool(
      ...
      type: ToolType.execute,
      parameters: {
        'package': ToolParameter(name: 'package', description: 'Package name', type: ToolParameterType.string, required: true),
        'manager': ToolParameter(name: 'manager', description: 'Package manager', type: ToolParameterType.string, options: ['pkg', 'apt', 'pip', 'npm']),
      },
      requiresAuth: false,
    ),
    Tool(
      ...
      type: ToolType.execute,
      parameters: {
        'service': ToolParameter(name: 'service', description: 'Service name', type: ToolParameterType.string, required: true),
      },
      requiresAuth: false,
    ),
    Tool(
    ...
    Tool(
      ...
      type: ToolType.read,
      parameters: {
        'path': ToolParameter(name: 'path', description: 'File path', type: ToolParameterType.string, required: true),
      },
      requiresAuth: false,
    ),
    Tool(
      ...
      parameters: {
        'path': ToolParameter(name: 'path', description: 'File path', type: ToolParameterType.string, required: true),
        'content': ToolParameter(name: 'content', description: 'File content', type: ToolParameterType.string, required: true),
      },
      requiresAuth: false,
    ),
    Tool(
    ...
    Tool(
      ...
      type: ToolType.read,
      parameters: {
        'path': ToolParameter(name: 'path', description: 'Directory path', type: ToolParameterType.string),
      },
      requiresAuth: false,
    ),
  ];
  ...
  static String _sanitizeInput(String input) {
    return input.replaceAll(_sanitizePattern, '').replaceAll('"', '').replaceAll("'", '');
  }

  static Future<SSHResult> _exec(String command, {int timeoutSeconds = 30}) async {
    ...
    try {
      ...
      if (result.isSuccess) {
        return ToolResult.success(
          result.stdout.isNotEmpty ? result.stdout : 'Command executed successfully',
          data: {
  ...
  static Future<ToolResult> _installPackage(Map<String, dynamic> params) async {
    try {
      ...
      if (result.isSuccess) {
        ...
        return ToolResult.failure('Failed to install package: ${result.stderr}');
      }
    } catch (e) {
  ...
  static Future<ToolResult> _writeFile(Map<String, dynamic> params) async {
    try {
      ...
      final result = await _exec("cat > '$escapedPath' << 'EOF'\n$escapedContent\nEOF");
  ...
  static Future<ToolResult> _fileExists(Map<String, dynamic> params) async {
    try {
      final path = params['path'];
      final escapedPath = path.replaceAll("'", "'\\''");
      final result = await _exec("test -e '$escapedPath' && echo 'exists' || echo 'not_exists'");
  ...
  static Future<ToolResult> _listDirectory(Map<String, dynamic> params) async {
    try {
      ...
      if (result.isSuccess) {
        final items = result.stdout.trim().split('\n').skip(1).map((line) {
