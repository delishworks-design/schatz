...
class CloudflarePlugin {
  ...
  static List<Tool> get tools => [
        ...
        Tool(
            id: 'kv.manage',
            name: 'Manage KV',
            description: 'Manage KV namespace',
            pluginId: id,
            type: ToolType.manage,
            parameters: {
              'action': ToolParameter(
    ...
    try {
      ...
      switch (action) {
        case 'list':
          ...
          return ToolResult.success('DNS records retrieved',
              data: {'records': response.data['result']});
        case 'create':
          final response = await client.post('/zones/$zoneId/dns_records',
              data: params['record']);
          return ToolResult.success('DNS record created',
              data: response.data['result']);
        case 'delete':
          await client
              .delete('/zones/$zoneId/dns_records/${params['recordId']}');
  ...
  static Future<ToolResult> _deployWorker(Map<String, dynamic> params) async {
    try {
      ...
      return ToolResult.success('Worker deployed: ${params['script']}',
          data: response.data);
    } catch (e) {
      return ToolResult.failure('Failed: $e');
    }
      ...
      switch (action) {
        case 'get':
          final response = await client.get(
              '/accounts/$accountId/kv/namespaces/$nsId/values/${params['key']}');
          return ToolResult.success('Value retrieved',
        ...
        case 'list':
          final response =
              await client.get('/accounts/$accountId/kv/namespaces/$nsId/keys');
          return ToolResult.success('Keys retrieved',
              data: {'keys': response.data['result']});
        default:
          return ToolResult.failure('Unknown action');
      }
    } catch (e) {
