...
class VercelPlugin {
  ...
  static List<Tool> get tools => [
    ...
    Tool(
      ...
      parameters: {
        'name': ToolParameter(name: 'name', description: 'Project name', type: ToolParameterType.string, required: true),
        'framework': ToolParameter(name: 'framework', description: 'Framework', type: ToolParameterType.string),
        'files': ToolParameter(name: 'files', description: 'Files to deploy', type: ToolParameterType.map),
      },
    ),
    Tool(
      id: 'logs',
      ...
      type: ToolType.read,
      parameters: {
        'deploymentId': ToolParameter(name: 'deploymentId', description: 'Deployment ID', type: ToolParameterType.string, required: true),
      },
    ),
    Tool(
      id: 'domain.manage',
      ...
      parameters: {
        'action': ToolParameter(name: 'action', description: 'Action', type: ToolParameterType.string, required: true, options: ['add', 'remove', 'list']),
        'domain': ToolParameter(name: 'domain', description: 'Domain name', type: ToolParameterType.string),
      },
    ),
    Tool(
      id: 'env.manage',
      ...
      type: ToolType.manage,
      parameters: {
        'action': ToolParameter(name: 'action', description: 'Action', type: ToolParameterType.string, required: true, options: ['add', 'update', 'remove', 'list']),
        'projectId': ToolParameter(name: 'projectId', description: 'Project ID', type: ToolParameterType.string, required: true),
        'key': ToolParameter(name: 'key', description: 'Variable key', type: ToolParameterType.string),
        'value': ToolParameter(name: 'value', description: 'Variable value', type: ToolParameterType.string),
      },
    ),
  ];
  ...
  static Future<ToolResult> _listProjects(Map<String, dynamic> params) async {
    try {
      ...
      final projects = (response.data['projects'] as List).take(10).map((p) => {
        'id': p['id'],
        'name': p['name'],
  ...
  static Future<ToolResult> _viewLogs(Map<String, dynamic> params) async {
    try {
      final client = await _getClient();
      final response = await client.get('/v2/deployments/${params['deploymentId']}/events');
  ...
  static Future<ToolResult> _manageDomains(Map<String, dynamic> params) async {
    try {
      ...
      switch (action) {
        ...
        case 'add':
          final response = await client.post('/v5/domains', data: {'name': params['domain']});
  ...
  static Future<ToolResult> _manageEnvVars(Map<String, dynamic> params) async {
    try {
      ...
      switch (action) {
        case 'list':
          final response = await client.get('/v9/projects/${params['projectId']}/env');
          return ToolResult.success(
            'Found ${response.data['envs'].length} variables',
            data: {'envVars': response.data['envs']},
          );
        case 'add':
          final response = await client.post('/v9/projects/${params['projectId']}/env', data: {
            'key': params['key'],
            'value': params['value'],
            'type': 'encrypted',
          });
          return ToolResult.success('Environment variable added: ${params['key']}');
        case 'remove':
          await client.delete('/v9/projects/${params['projectId']}/env/${params['key']}');
          return ToolResult.success('Environment variable removed: ${params['key']}');
        default:
          return ToolResult.failure('Unknown action: $action');
      }
    } catch (e) {
