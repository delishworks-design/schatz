...
class GitHubPlugin {
  ...
  static const String description = 'Interact with GitHub repositories, issues, and pull requests';
  ...
  static List<Tool> get tools => [
    Tool(
      ...
      type: ToolType.read,
      parameters: {
        'owner': ToolParameter(name: 'owner', description: 'Repository owner', type: ToolParameterType.string, required: true),
        'repo': ToolParameter(name: 'repo', description: 'Repository name', type: ToolParameterType.string, required: true),
        'path': ToolParameter(name: 'path', description: 'File path (optional)', type: ToolParameterType.string),
      },
    ),
    Tool(
      id: 'search',
      ...
      parameters: {
        'query': ToolParameter(name: 'query', description: 'Search query', type: ToolParameterType.string, required: true),
        'repo': ToolParameter(name: 'repo', description: 'Limit to repo (owner/repo)', type: ToolParameterType.string),
      },
    ),
    Tool(
      id: 'issue.list',
      ...
      parameters: {
        'owner': ToolParameter(name: 'owner', description: 'Repository owner', type: ToolParameterType.string, required: true),
        'repo': ToolParameter(name: 'repo', description: 'Repository name', type: ToolParameterType.string, required: true),
        'state': ToolParameter(name: 'state', description: 'Issue state', type: ToolParameterType.string, options: ['open', 'closed', 'all']),
      },
    ),
    Tool(
      id: 'issue.create',
      ...
      parameters: {
        'owner': ToolParameter(name: 'owner', description: 'Repository owner', type: ToolParameterType.string, required: true),
        'repo': ToolParameter(name: 'repo', description: 'Repository name', type: ToolParameterType.string, required: true),
        'title': ToolParameter(name: 'title', description: 'Issue title', type: ToolParameterType.string, required: true),
        'body': ToolParameter(name: 'body', description: 'Issue body', type: ToolParameterType.string),
        'labels': ToolParameter(name: 'labels', description: 'Labels', type: ToolParameterType.list),
      },
    ),
    Tool(
      id: 'pr.list',
      ...
      parameters: {
        'owner': ToolParameter(name: 'owner', description: 'Repository owner', type: ToolParameterType.string, required: true),
        'repo': ToolParameter(name: 'repo', description: 'Repository name', type: ToolParameterType.string, required: true),
        'state': ToolParameter(name: 'state', description: 'PR state', type: ToolParameterType.string, options: ['open', 'closed', 'all']),
      },
    ),
    Tool(
      id: 'pr.create',
      ...
      parameters: {
        'owner': ToolParameter(name: 'owner', description: 'Repository owner', type: ToolParameterType.string, required: true),
        'repo': ToolParameter(name: 'repo', description: 'Repository name', type: ToolParameterType.string, required: true),
        'title': ToolParameter(name: 'title', description: 'PR title', type: ToolParameterType.string, required: true),
        'head': ToolParameter(name: 'head', description: 'Head branch', type: ToolParameterType.string, required: true),
        'base': ToolParameter(name: 'base', description: 'Base branch', type: ToolParameterType.string, required: true),
        'body': ToolParameter(name: 'body', description: 'PR body', type: ToolParameterType.string),
      },
    ),
    Tool(
      id: 'deploy',
      ...
      parameters: {
        'owner': ToolParameter(name: 'owner', description: 'Repository owner', type: ToolParameterType.string, required: true),
        'repo': ToolParameter(name: 'repo', description: 'Repository name', type: ToolParameterType.string, required: true),
        'workflow': ToolParameter(name: 'workflow', description: 'Workflow file name', type: ToolParameterType.string, required: true),
        'ref': ToolParameter(name: 'ref', description: 'Branch or tag', type: ToolParameterType.string),
      },
    ),
  ];
  ...
  static Future<ToolResult> _readRepo(Map<String, dynamic> params) async {
    try {
      ...
      final response = await client.get('/repos/${params['owner']}/${params['repo']}');
  ...
  static Future<ToolResult> _searchCode(Map<String, dynamic> params) async {
    try {
      ...
      final response = await client.get('/search/code', queryParameters: {'q': query});
  ...
  static Future<ToolResult> _listIssues(Map<String, dynamic> params) async {
    try {
      ...
      final issues = (response.data as List?)?.take(10).map((issue) => {
        'number': issue['number'],
        'title': issue['title'],
        'state': issue['state'],
  ...
  static Future<ToolResult> _listPrs(Map<String, dynamic> params) async {
    try {
      ...
      final prs = (response.data as List?)?.take(10).map((pr) => {
        'number': pr['number'],
        'title': pr['title'],
        'state': pr['state'],
        'user': pr['user']?['login'],
