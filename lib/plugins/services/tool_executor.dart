...
class ToolExecutorService {
  ...
  Future<ToolResult> execute(String fullName, Map<String, dynamic> params) async {
    ...
    try {
      ...
      if (tool.requiresAuth) {
        ...
        if (!hasAuth) {
          stopwatch.stop();
          return ToolResult.failure('Authentication required for: ${tool.pluginId}');
        }
      }
  ...
  String? _validateParams(Tool tool, Map<String, dynamic> params) {
    for (final entry in tool.parameters.entries) {
      ...
      if (value != null && param.options != null && !param.options!.contains(value.toString())) {
