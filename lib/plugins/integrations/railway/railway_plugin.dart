...
class RailwayPlugin {
  ...
  static Future<ToolResult> _listServices(Map<String, dynamic> params) async {
    try {
      ...
      final response = await client.post('', data: {
        'query':
            '{ projects(first: 10) { edges { node { name services { edges { node { id name } } } } } } }'
      });
      return ToolResult.success('Services retrieved',
  ...
  static Future<ToolResult> _deploy(Map<String, dynamic> params) async {
    try {
      ...
      final response = await client.post('', data: {
        'query':
            'mutation (\$serviceId: ID!) { serviceDeploy(input: { serviceId: \$serviceId }) { id } }',
        'variables': {'serviceId': params['serviceId']},
      });
      return ToolResult.success('Deployment triggered',
          data: response.data['data']);
    } catch (e) {
      return ToolResult.failure('Failed: $e');
    }
  ...
  static Future<ToolResult> _viewLogs(Map<String, dynamic> params) async {
    try {
      ...
      final response = await client.post('', data: {
        'query':
            'query (\$serviceId: ID!) { logs(serviceId: \$serviceId, first: 50) { edges { node { timestamp message } } } }',
        'variables': {'serviceId': params['serviceId']},
      });
  ...
  static Future<ToolResult> _manageEnv(Map<String, dynamic> params) async {
    try {
      ...
      if (action == 'list') {
        final response = await client.post('', data: {
          'query':
              'query (\$serviceId: ID!) { variables(serviceId: \$serviceId) { key value } }',
          'variables': {'serviceId': params['serviceId']},
        });
        return ToolResult.success('Variables retrieved',
            data: response.data['data']);
      }
